...
from gb.model.mlp import TapeableLinear


'''Adapted from https://github.com/gordicaleksa/pytorch-GAT/tree/main'''

class RGAT(TapeableModule, StandardFittable):
    """
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, add_skip_connection=True, bias=True,
                 dropout=0.6, log_attention_weights=False, params=None):
        super().__init__()
        assert num_of_layers == len(num_heads_per_layer) == len(num_features_per_layer) - 1, f'Enter valid arch params.'

        GATLayer = GATLayerImp2
        num_heads_per_layer = [1] + num_heads_per_layer  # trick - so that I can nicely create GAT layers below

        self.gat_layers = nn.ModuleList([])  # collect GAT layers
...
class GATLayer(TapeableModule):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
        ...
        self.linear_proj = TapeableLinear(num_in_features, num_of_heads * num_out_features, bias=False)
        ...
        self.scoring_fn_target = TapeableParameter(torch.Tensor(1, num_of_heads, num_out_features))
        self.scoring_fn_source = TapeableParameter(torch.Tensor(1, num_of_heads, num_out_features))

        # Bias is definitely not crucial to GAT - feel free to experiment (I pinged the main author, Petar, on this one)
        if bias and concat:
            self.bias = TapeableParameter(torch.Tensor(num_of_heads * num_out_features))
        elif bias and not concat:
            self.bias = TapeableParameter(torch.Tensor(num_out_features))
        else:
            pass
            #self.register_parameter('bias', None)

        if add_skip_connection:
            self.skip_proj = TapeableLinear(num_in_features, num_of_heads * num_out_features, bias=False)
        else:
            pass
...
class GATLayerImp3(GATLayer):
    ...
    def forward(self, A, X):
        ...
        #
        '''Not supposed to use edge idx'''
        in_nodes_features, edge_index = X, A.nonzero().t()  # unpack data
...
class GATLayerImp2(GATLayer):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
                 dropout_prob=0.6, add_skip_connection=True, bias=True, log_attention_weights=False, params=None):
        ...
        if params is None:
            ...
            self.robust_sum = RobustSum(**params)
    ...
    def forward(self, A, X):
        ...
        in_nodes_features, connectivity_mask = X, A  # unpack data
        ...
        # connectivity mask will put a large number on all locations where there are edges, after applying the softmax
        # this will result in attention scores being computed only for existing edges
        # all_attention_coefficients = self.softmax(all_scores + connectivity_mask * 8) # too large exp will cause the grad to be unstable...

        # replace with soft approximation to allow correct grad through
        all_attention_coefficients = torch.exp(all_scores)
        all_attention_coefficients = connectivity_mask * all_attention_coefficients
        all_attention_coefficients = all_attention_coefficients / (all_attention_coefficients.sum(-1).unsqueeze(-1) + 1e-3) # normalize again
        #
        ...
        out_nodes_features = self.robust_sum(all_attention_coefficients, nodes_features_proj.transpose(0, 1))
